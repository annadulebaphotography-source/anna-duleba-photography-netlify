...
function transformHtmlFile(file) {
  ...
  if (hasBlogArticlePage(file)) {
    html = appendStylesheet(html, `${prefix}css/netlify-local-cms.css?v=netlify-blog-phase1`);
    html = appendScript(html, `${prefix}js/netlify-blog-cms.js?v=netlify-blog-phase1`);
  }
